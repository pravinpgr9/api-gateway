...
Ensure Consul is running on `localhost:8500`. Start Consul in development mode:
```sh
$ consul agent -dev
```
...
To test rate limiting on Linux/macOS:
```sh
...
```powershell
1..110 | ForEach-Object { curl.exe -X GET http://localhost:3000/api/users }
```
